  ...
  const colors = [
    {
      ...
      text: "text-blue-900",
      avgBg: "bg-blue-100/60",
    },
    {
      bg: "bg-emerald-50/40",
      border: "border-emerald-100",
      text: "text-emerald-900",
      avgBg: "bg-emerald-100/60",
    },
    {
      bg: "bg-amber-50/40",
      border: "border-amber-100",
      text: "text-amber-900",
      avgBg: "bg-amber-100/60",
    },
    {
      bg: "bg-purple-50/40",
      border: "border-purple-100",
      text: "text-purple-900",
      avgBg: "bg-purple-100/60",
    },
    {
      bg: "bg-rose-50/40",
      border: "border-rose-100",
      text: "text-rose-900",
      avgBg: "bg-rose-100/60",
    },
    {
      bg: "bg-cyan-50/40",
      border: "border-cyan-100",
      text: "text-cyan-900",
      avgBg: "bg-cyan-100/60",
    },
  ];
...
export default function GradeTable({
  ...
  level.subjects.forEach((subject, subjectIndex) => {
    ...
    if (isHidden) {
      ...
      activeCategories.forEach((category) => {
        ...
        // +1 for the Average column only if itemCount > 1 and not hidden
        const catSpan = isCatHidden
          ? 1
          : category.itemCount + (category.itemCount > 1 ? 1 : 0);
        categoryCols.push({
          category,
          colSpan: catSpan,
          subjectId: subject.id,
        ...
        if (!isCatHidden) {
          for (let i = 0; i < category.itemCount; i++) {
            itemCols.push({
              ...
              subjectId: subject.id,
              itemIndex: i,
              label: category.itemCount === 1 ? "Score" : "",
  ...
  const studentMetrics = useMemo(() => {
    const scores = students.map((student) => {
      ...
      level.subjects.forEach((subject) => {
        ...
        activeCategories.forEach((category) => {
          ...
          const categoryPercentage =
            categoryMax > 0 ? categoryEarned / categoryMax : 0;
        ...
        const subjectScorePercentage =
          subjectCatWeightSum > 0
            ? (subjectPointsEarned / subjectCatWeightSum) * 100
            : 0;
        subjectScores[subject.id] = subjectScorePercentage;

        const subjectTargetWeight =
          subject.targetWeight !== undefined && subject.targetWeight > 0
      ...
      const finalScore =
        totalSubjectWeight > 0 ? weightedSubjectSum / totalSubjectWeight : 0;
  ...
  return (
    <div className="overflow-x-auto custom-scrollbar">
      <table className="w-full text-left text-sm text-slate-600 border-collapse min-w-max">
        <thead
          className={`text-xs uppercase border-b border-slate-200 ${currentPaper.textClass}`}
        >
          {/* Row 1: Subjects */}
          <tr>
            <th
              rowSpan={3}
              className={`px-2 py-2 font-semibold border-r border-slate-200 w-10 text-center sticky left-0 z-20 shadow-[1px_0_0_0_#e2e8f0] ${currentPaper.bgClass}`}
            ...
            <th
              rowSpan={3}
              className={`px-2 py-2 font-semibold border-r border-slate-200 w-[160px] min-w-[160px] max-w-[160px] sticky left-10 z-20 shadow-[1px_0_0_0_#e2e8f0] cursor-pointer hover:bg-black/[0.05] transition-colors ${currentPaper.bgClass}`}
            ...
            <th
              rowSpan={3}
              className="px-1 py-2 font-semibold border-r border-slate-200 text-center w-16 text-[10px]"
            >
              Attnd%
            </th>
            {subjectCols.map((sc) => {
              ...
              return (
                <th
                  key={sc.subject.id}
                  colSpan={sc.colSpan}
                  className={`px-4 py-2 font-semibold border-r border-b text-center ${sc.isHidden ? "bg-slate-200 border-slate-300 text-slate-500" : `${theme.bg} ${theme.border} ${theme.text}`}`}
                >
                  <div className="flex items-center justify-center gap-2">
                    <span>
                      ...
                      <span className="opacity-75 font-normal">
                        (
                        {sc.subject.targetWeight !== undefined &&
                        sc.subject.targetWeight > 0
                          ? sc.subject.targetWeight
                          : getSubjectWeight(sc.subject)}
                        %)
                      </span>
                    </span>
                    <button
            ...
            <th
              rowSpan={3}
              className="px-4 py-3 font-semibold bg-blue-50 border-l border-blue-100 text-center shadow-[-1px_0_0_0_#dbeafe] cursor-pointer hover:bg-blue-100 transition-colors"
              onClick={() => handleSort("finalScore")}
            >
              <div className="flex items-center justify-center gap-1">
                Total Avg <ArrowUpDown className="w-3 h-3 text-blue-400" />
              </div>
            </th>
            <th
              rowSpan={3}
              className="px-4 py-3 font-semibold bg-blue-50 border-l border-blue-100 text-center cursor-pointer hover:bg-blue-100 transition-colors"
            ...
            <th
              rowSpan={3}
              className="px-4 py-3 font-semibold bg-blue-50 border-l border-blue-100 text-center"
            >
              Grade
            </th>
            <th
            ...
            <th
              rowSpan={3}
              className="px-4 py-3 font-semibold border-l border-slate-200 text-center w-48"
            >
              Comment
            </th>
            <th
              rowSpan={3}
              className="px-4 py-3 font-semibold text-center border-l border-slate-200 w-16"
            >
              Act
            </th>
          </tr>
            ...
            {categoryCols.map((cc, i) => {
              ...
              return (
                <th
                  ...
                  colSpan={cc.colSpan}
                  className={`px-2 py-2 font-semibold border-r border-b text-center ${theme.bg} ${theme.border} ${theme.text} whitespace-nowrap`}
                >
                  <div className="flex items-center justify-center gap-1.5">
                    <span>
                      {cc.category.name}{" "}
                      <span className="opacity-75 normal-case font-normal ml-0.5">
                        ({cc.category.weight}%)
                      </span>
                    </span>
                    <button
            ...
            {itemCols.map((ic, i) => {
              ...
              return (
                <th
                  ...
                  className={`px-1 py-1 font-medium border-r border-b text-center ${ic.isAvg ? `${theme.avgBg} ${theme.text} w-20 ${theme.border}` : `${theme.bg} ${theme.text} w-16 ${theme.border}`}`}
                >
                  {ic.isAvg ? (
                    <div className="flex flex-col items-center justify-center h-full">
                      <span className="font-semibold">{ic.label}</span>
                      <span className="text-[10px] normal-case opacity-75">
                        {ic.itemIndex === -3 ? "Subject" : "Weighted"}
                      </span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center">
                      <span className="text-[10px] font-semibold opacity-75 mb-0.5">
                        {ic.label || `#${ic.itemIndex + 1}`}
                      </span>
          ...
          {sortedStudents.map((student, index) => {
            ...
            let status =
              finalScore >= 70
                ? "Pass"
                : finalScore < 50
                  ? "Fail + Support 1 Month"
                  : "Fail";

            const attendanceVal = parseFloat(student.attendance || "100");
            const isAttendanceFail =
              !isNaN(attendanceVal) && attendanceVal < 70;
            if (isAttendanceFail) {
              status = "Auto-Fail";
              grade = "F";
            }
            ...
            return (
              <tr
                key={student.id}
                className="hover:bg-black/[0.02] transition-colors even:bg-black/[0.01] group"
              >
                <td
                  className={`px-1 py-1 border-r border-slate-100 text-center font-medium text-slate-400 sticky left-0 z-10 shadow-[1px_0_0_0_#f1f5f9] ${currentPaper.bgClass}`}
                  style={{ ...currentPaper.customStyle, left: 0 }}
                >
                  {index + 1}
                </td>
                <td
                  className={`px-2 py-1 border-r border-slate-100 sticky left-10 z-10 shadow-[1px_0_0_0_#f1f5f9] transition-colors w-[160px] min-w-[160px] max-w-[160px] ${currentPaper.bgClass}`}
                  style={{ ...currentPaper.customStyle, left: "2.5rem" }}
                >
                  <input
                    type="text"
                ...
                {itemCols.map((ic, i) => {
                  ...
                  if (ic.itemIndex === -3) {
                    ...
                    return (
                      <td
                        key={`${ic.categoryId}_subavg_${i}`}
                        className={`px-1 py-1 border-r border-b border-slate-100 font-bold text-center text-sm ${theme.avgBg} ${theme.text}`}
                      >
                        {subAvg.toFixed(1)}%
                      </td>
                    );
                  ...
                  if (ic.isAvg) {
                    ...
                    return (
                      <td
                        ...
                        className={`px-1 py-1 border-r border-b border-slate-100 font-medium text-center text-sm ${theme.avgBg} ${theme.text}`}
                      >
                        {avg.toFixed(1)}%
                      </td>
                    );
                  ...
                  return (
                    <td
                      key={scoreKey}
                      className={`px-0.5 py-1 border-r border-b border-slate-100 ${theme.bg}`}
                    >
                      <input
                        type="number"
                        min="0"
                ...
                <td className="px-2 py-1 bg-blue-50/30 text-center border-l border-blue-100/50">
                  <span
                    className={`text-xs font-semibold ${status === "Pass" ? "text-green-600" : "text-red-500"}`}
                  >
                    {status}
                  </span>
                </td>
                <td className="px-1 py-1 border-l border-slate-100 transition-colors">
                  <input
                    type="text"
                    value={
                      isAttendanceFail ? "Auto-Fail" : student.comment || ""
                    }
                    onChange={(e) =>
                      onUpdateStudentField(
                        student.id,
                ...
                <td className="px-1 py-1 text-center border-l border-slate-100 transition-colors">
                  <button
