...
import { Level, Student, calculateGrade, getSubjectWeight, PAPER_STYLES } from '../types';
...
export default function GradeTable({ level, onUpdateLevel, students, onUpdateStudent, onUpdateStudentField, onDeleteStudent, resultMode, paperStyle }: Props) {
  ...
  return (
    <div className="overflow-x-auto custom-scrollbar">
      <table className="w-full text-left text-sm text-slate-600 border-collapse min-w-max">
        <thead className={`text-xs uppercase border-b border-slate-200 ${currentPaper.textClass}`}>
          {/* Row 1: Subjects */}
          <tr>
            <th 
              rowSpan={3} 
            ...
            {subjectCols.map(sc => {
              ...
              <th key={sc.subject.id} colSpan={sc.colSpan} className={`px-4 py-2 font-semibold border-r border-b text-center ${sc.isHidden ? 'bg-slate-200 border-slate-300 text-slate-500' : `${theme.bg} ${theme.border} ${theme.text}`}`}>
                <div className="flex items-center justify-center gap-2">
                  <span>{sc.subject.name} <span className="opacity-75 font-normal">({sc.subject.targetWeight !== undefined && sc.subject.targetWeight > 0 ? sc.subject.targetWeight : getSubjectWeight(sc.subject)}%)</span></span>
        ...
        <tbody className="divide-y divide-slate-100">
          {sortedStudents.map((student, index) => {
            ...
            return (
              <tr key={student.id} className="hover:bg-black/[0.02] transition-colors even:bg-black/[0.01] group">
                <td 
